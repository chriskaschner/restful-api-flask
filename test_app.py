...
import json
import base64
username = 'ReturnPath'.encode("utf-8")
password = 'python'.encode("utf-8")

# User_Password_Encoded = base64.b64encode("{0}:{1}".format(username, password))
User_Password_Encoded = ("{0}:{1}".format(username, password))
User_Password_Encoded = User_Password_Encoded.encode()
...
class TestApp(unittest.TestCase):
    ...
    def test_create_new_image(self):
        ...
        rv = self.app.post(
            '/img/api/v1.0/images',
            data=json.dumps(
                dict(url='http://imgdirect.s3-website-us-west-2.'
                     'amazonaws.com/neither.jpg')),
    ...
    def test_create_not_json_new_image(self):
        ...
        rv = self.app.post(
            '/img/api/v1.0/images',
            data='http://imgdirect.s3-website-us-west-2.'
            'amazonaws.com/neither.jpg',
            content_type='application/json',
            headers=headers)
